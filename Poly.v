...
Lemma map_distr : forall X Y (f : X -> Y) (l1 l2 : list X),
...
Example fold_useful :
  fold (fun nums n => fold plus nums n) [[1];[];[2;3];[4]] 0 = 10.
Proof. reflexivity. Qed.
(** Sum all the numbers that in lists of lists *)
...
Proof.
  intros. replace (fold_length l) with (fold (fun _ n => S n) l 0). induction l.
  - reflexivity.
...
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  fold (fun x ly => f x::ly) l (@nil Y).

Example test_foldmap:
...
Proof.
  intros. replace (fold_map f l) with (fold (fun x ly => f x::ly) l (@nil Y)).
  induction l.
  - reflexivity.
  - simpl. rewrite IHl. reflexivity.
  - reflexivity.
...
(** Recall the definition of the [nth_error] function:
   ...
   forall X n l, length l = n -> @nth_error X l n = None
    ...
    - Next, suppose [l = cons h l'], where the inductive hypothesis [IHl'] is

        forall n: nat, length l' = n -> nth_error X l' n = None. 

      We must show

        forall n: nat, length (h::l') = n -> nth_error X (h::l') n = None.

      We introduce 
      
        length (h::l') = n
        
      as the hypothesis [H] and we must know
      
        nth_error X (h::l') n = None.
      
      for any [n : nat].
      
      We then destruct on [n].

        + First, suppose [n = O]. 
        
          By definition of [length], we get
          
            length (h::l') = S (length l') != n.
    
          A contradiction arrises, so this case can be proved immediately.
          
        + Next, suppose [n = S n'].
        
           By definition of [length],
            
            length (h::l') = S n' -> S (length l') = S n'.
            
          Due to the injective property of  [nat] constructor,
          ...
          By definition of [nth_error],
          
            nth_error X (h::l') (S n') = None -> nth_error X l' n' = None.
            
          which is immediately proved.
